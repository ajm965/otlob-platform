# location/infrastructure

## Purpose

Adapters for persistence, external APIs, and cache for `location`.

## Responsibilities

- Repository implementations, datasources, models, mappers, cache

## Dependencies

- `location/application` interfaces and `location/domain` ports
- Firebase/PSP/SMS SDKs only when implementation is authorized later

## Ownership

Backend infrastructure owners for `location`

## Future Implementation Notes

- No Firebase configuration or SDK wiring in Phase 2.1
