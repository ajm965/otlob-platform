# categories/infrastructure

## Purpose

Adapters for persistence, external APIs, and cache for `categories`.

## Responsibilities

- Repository implementations, datasources, models, mappers, cache

## Dependencies

- `categories/application` interfaces and `categories/domain` ports
- Firebase/PSP/SMS SDKs only when implementation is authorized later

## Ownership

Backend infrastructure owners for `categories`

## Future Implementation Notes

- No Firebase configuration or SDK wiring in Phase 2.1
