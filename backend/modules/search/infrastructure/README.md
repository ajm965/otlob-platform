# search/infrastructure

## Purpose

Adapters for persistence, external APIs, and cache for `search`.

## Responsibilities

- Repository implementations, datasources, models, mappers, cache

## Dependencies

- `search/application` interfaces and `search/domain` ports
- Firebase/PSP/SMS SDKs only when implementation is authorized later

## Ownership

Backend infrastructure owners for `search`

## Future Implementation Notes

- No Firebase configuration or SDK wiring in Phase 2.1
